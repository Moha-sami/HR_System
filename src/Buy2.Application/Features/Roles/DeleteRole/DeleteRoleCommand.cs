...
public class DeleteRoleCommandHandler : IRequestHandler<DeleteRoleCommand, bool>
{
    ...
    public async Task<bool> Handle(DeleteRoleCommand request, CancellationToken cancellationToken)
    {
        ...
        var isAssigned = await _employeeRepository.AnyAsync(e => e.RoleId == request.RoleId, cancellationToken);

        if (isAssigned)
        {
            throw new InvalidOperationException($"Role '{role.Name}' is assigned to active employees and cannot be deleted.");
        }
