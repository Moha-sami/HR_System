...
public class GetQualificationsQueryHandler : IRequestHandler<GetQualificationsQuery, IEnumerable<QualificationLookupDto>>
{
    ...
    {
        var spec = new Specification<Qualification>();

        if (!string.IsNullOrWhiteSpace(request.Search))
        {
            var searchLower = request.Search.Trim().ToLower();
            spec.Where(q => q.Name.ToLower().Contains(searchLower) || (q.Description != null && q.Description.ToLower().Contains(searchLower)));
        }

        if (!string.IsNullOrWhiteSpace(request.Type))
        {
            var typeLower = request.Type.Trim().ToLower();
            spec.Where(q => q.Category.ToLower() == typeLower);
        }

        return await _qualificationRepository.ListAsync(
            spec,
            q => new QualificationLookupDto(
                q.Id,
                q.Name,
                q.Category,
                q.Description,
