...
public class GenericRepository<T> : IRepository<T> where T : class
{
    ...
    // TEMPORARY for the stacked review of SCRUM-389: keeps pre-existing
    // Query()-based handlers compiling until the follow-up (SCRUM-390)
    // migrates them and removes this member.
    public IQueryable<T> Query(bool asNoTracking = true)
    {
        return BaseQuery(tracked: !asNoTracking, ignoreQueryFilters: false);
    }
